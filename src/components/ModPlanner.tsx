...
function ModRow({ mod, detail, resolvedVersion, disabled, onChange, onDelete }: { mod: ConfiguredMod; detail?: ModDetails[number]; resolvedVersion?: string; disabled: boolean; onChange(value: ConfiguredMod): void; onDelete(): void }) {
  ...
  return <div className={`grid min-h-[70px] grid-cols-[minmax(220px,1fr)_150px_auto_auto_auto] items-center gap-[7px] border-b border-[#1e2320] py-2 pr-3 pl-[18px] max-[850px]:grid-cols-[1fr_auto_auto_auto] ${mod.enabled ? '' : 'opacity-55'}`}>
    ...
    <button className={`size-[34px] p-0 ${mod.version ? 'text-[#6ee7a0]' : 'text-[#fbbf24]'}`} aria-label={mod.version ? `解锁 ${mod.name}` : `锁定 ${mod.name}`} title={mod.version ? 'Unlock version' : 'Lock resolved version'} disabled={disabled || (!mod.version && !resolvedVersion)} onClick={() => mod.version ? onChange({ ...mod, version: undefined }) : lock()}>{mod.version ? <Unlock size={14} /> : <Lock size={14} />}</button>
    <button className={`size-[34px] p-0 ${mod.enabled ? 'text-[#6ee7a0]' : 'text-[#94a3b8]'}`} aria-label={`${mod.enabled ? '禁用' : '启用'} ${mod.name}`} title={mod.enabled ? 'Disable' : 'Enable'} disabled={disabled} onClick={() => onChange({ ...mod, enabled: !mod.enabled })}><Power size={15} /></button>
