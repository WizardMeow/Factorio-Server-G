...
export function SaveManager({ saves, busy, running, onAction, onUpload }: Props) {
  ...
  return <section className="panel mt-4 overflow-hidden">
    <PanelHeader eyebrow="RECOVERY" title="Saves & Backups">
      <label className="upload h-[34px]"><Upload size={15} />Import .zip<input className="hidden" type="file" accept=".zip" onChange={event => onUpload(event.target.files?.[0])} /></label>
    </PanelHeader>
...
function SaveList({ kind, title, entries, onAction, download, disabled, selected, removable }: { kind: 'autosaves' | 'imports' | 'backups'; title: string; entries: SaveEntry[]; onAction(path: string, body: unknown): void; download?: boolean; disabled?: boolean; selected?: string; removable?: boolean }) {
  ...
  return <div><h4 className="mt-0 mb-[11px] text-[11px] tracking-[.08em] text-[#8c9690] uppercase">{title}<span className="float-right rounded-[9px] bg-[#252a27] px-1.5 py-px font-mono text-[9px]">{entries.length}</span></h4><div className="flex flex-col gap-[7px]">
    {entries.length ? entries.map(entry => <div className="flex h-[54px] items-center justify-between gap-1.5 rounded-[7px] border border-[#282e2b] bg-[#0c0f0e] px-[9px] py-2" key={entry.name}>
      ...
      <div className="flex gap-1">{download && <a className={actionClass} aria-label={`下载 ${entry.name}`} href={`/api/saves/backups/${encodeURIComponent(entry.name)}`}><Download size={14} /></a>}
